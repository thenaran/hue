...
class Bridge(object):
  ...
  def __init__(self, ip=None, username=None):
    ...
    if os.access(os.getenv(USER_HOME), os.W_OK):
      self.config_file_path = os.path.join(
        os.getenv(USER_HOME), '.prota_hue')
    else:
      self.config_file_path = os.path.join(os.getcwd(), '.prota_hue')
    ...
    try:
      ...
      logging.exception("error connecting to the bridge.")
  ...
  def register_app(self):
    ...
    registration_request = {"devicetype": "prota_hue"}
